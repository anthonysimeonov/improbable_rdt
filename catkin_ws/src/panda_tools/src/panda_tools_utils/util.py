...
def quat2list(panda_quat):
    quat_list = [panda_quat.x, panda_quat.y, panda_quat.z, panda_quat.w]
    return quat_list

def panda_full_pose(panda_pose):
    pos_list = panda_pose['position'].tolist()
    ori_list = quat2list(panda_pose['orientation'])
    return pos_list + ori_list

# Estimate rigid transform with SVD (from Nghia Ho)
def register_corresponding_points(A, B):
    assert len(A) == len(B)
...
def rotate_local_pose(pose_world, offset):
    ...
    T = np.zeros((4, 4,))
    T[-1, -1] = 1
    T[:3, :3] = R.from_euler('xyz', [angle_x, angle_y, angle_z]).as_matrix()
