...
def Visualize(pipelines):
    ...
    for device, pipe in pipelines:
        ...
        show_images = np.hstack(
            (cv.cvtColor(color_image, cv.COLOR_RGB2BGR), depth_colormap)
        )
...
def main(args):
    ...
    if args.rs_config is not None:
        rs_config_fname = osp.join(
            path_util.get_rrp_config(), "real_cam_cfgs", args.rs_config
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--find_devices",
        action="store_true",
        help="If True, look for whatever devices are connected. Else, use the serial numbers from the config",
    )
    parser.add_argument(
        "--rs_config",
        type=str,
        default=None,
        help="Can provide a .yaml file located in the src/rdt/config/real_cam_cfgs directory",
    )
