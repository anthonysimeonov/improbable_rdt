...
from rdt.common.real_util import (
    RealCamInfoLCMSubscriber,
    RealCompressedCombinedImageLCMSubscriber,
)
...
def get_realsense_rgbd_subscribers(lc, rs_cfg):
    ...
    depth_sub_names = [
        f"{cam_name}_{depth_topic_name_suffix}" for cam_name in camera_names
    ]
    info_sub_names = [
        f"{cam_name}_{info_topic_name_suffix}" for cam_name in camera_names
    ]
    pose_sub_names = [
        f"{cam_name}_{pose_topic_name_suffix}" for cam_name in camera_names
    ]

    img_subscribers = []
    for i, name in enumerate(camera_names):
        # img_sub = RealImageLCMSubscriber(lc, rgb_sub_names[i], depth_sub_names[i])
        img_sub = RealCompressedCombinedImageLCMSubscriber(
            lc, rgb_sub_names[i], depth_sub_names[i]
        )
