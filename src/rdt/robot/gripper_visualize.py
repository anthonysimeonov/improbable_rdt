...
class PandaHand:
    def __init__(self, grasp_frame=False):
        # self.hand_mesh_path = osp.join(path_util.get_rdt_descriptions(), 'franka_panda/meshes/visual/hand.obj')
        self.hand_mesh_path = osp.join(path_util.get_rdt_descriptions(), 'franka_panda/meshes/collision/hand.obj')
        self.finger_mesh_path = osp.join(path_util.get_rdt_descriptions(), 'franka_panda/meshes/visual/finger.obj') 
...
class Robotiq2F140Hand:
    def __init__(self, grasp_frame=True):
        # self.hand_mesh_path = osp.join(path_util.get_rdt_descriptions(), 'franka_panda/meshes/visual/hand.obj')
        self.full_hand_mesh_path = osp.join(path_util.get_rdt_descriptions(), 'franka_panda/meshes/robotiq_2f140/full_hand_2f140.obj')
...
if __name__ == "__main__":
    mc_port = 6001 
    mc_vis = meshcat.Visualizer(f'tcp://127.0.0.1:{mc_port}')
    mc_vis['scene'].delete()

    panda_hand = PandaHand(grasp_frame=False)
    panda_hand.reset_pose()
    panda_hand_pose = np.eye(4); panda_hand_pose[:-1, -1] = np.array([0.0, 0.0, 0.5])
    panda_hand.transform_hand(panda_hand_pose)
    panda_hand.meshcat_show(mc_vis, name_prefix='panda_hand')

    robotiq_hand = Robotiq2F140Hand(grasp_frame=False)
    robotiq_hand.reset_pose()
    robotiq_hand_pose = np.eye(4); robotiq_hand_pose[:-1, -1] = np.array([0.0, 0.0, -0.5])
    robotiq_hand.transform_hand(robotiq_hand_pose)
    robotiq_hand.meshcat_show(mc_vis, name_prefix='robotiq_hand')
