...
def exists_and_true(d: dict, st: str):
    if st in d:
        if d[st]:
            return True 
    return False
...
def hasattr_and_not_none(o, st: str):
    if hasattr(o, st):
        attr = getattr(o, st)
        if attr is not None:
            return True
...
def crop_pcd(raw_pts, x=[0.0, 0.7], y=[-0.4, 0.4], z=[0.9, 1.5], return_idx=False):
    npw = np.where(
            (raw_pts[:, 0] > min(x)) & (raw_pts[:, 0] < max(x)) &
            (raw_pts[:, 1] > min(y)) & (raw_pts[:, 1] < max(y)) &
            (raw_pts[:, 2] > min(z)) & (raw_pts[:, 2] < max(z)))
    if return_idx:
        return raw_pts[npw[0], :], npw[0]
...
def rotate_pcd_center(pcd, rot_mat, pcd_ref=None, leave_centered=False, pcd_mean=None):
    """
...
def form_tf_mat_cent_pcd_rot(tf_mat, pcd):
...
def downsample_pcd_perm(pcd, n_pts, rix=None, return_perm=False):
    if rix is None:
